        ...
        <div class="w-11/12 mx-auto flex space-x-6 pt-10">
            <div class="w-full lg:w-8/12">
                <div class="bg-white mb-6">
                    ...
                    <div class="p-5">
                        <form action="/job-seeker/education" method="POST">
                            @csrf
                            <div class="w-full flex flex-col space-y-3 md:space-y-0 md:flex-row md:space-x-6 mb-3">
                                ...
                                <div class="w-full md:w-1/2">
                                    ...
                                    <select name="minimum_qualification" id="minimum_qualification" class="h-10 py-1 px-2 rounded-md border border-gray-300 hover:border-gray-400 outline-none text-gray-500 placeholder-gray-400 w-full block mt-1">
                                        ...
                                        <option value="Degree" {{ old('minimum_qualification') == 'Degree' ? 'selected' : '' }}>Degree</option>
                                        <option value="Diploma" {{ old('minimum_qualification') == 'Diploma' ? 'selected' : '' }}>Diploma</option>
                                        <option value="High-School (S.S.C.E)" {{ old('minimum_qualification') == 'High-School (S.S.C.E)' ? 'selected' : '' }}>High-School (S.S.C.E)</option>
                                        <option value="HND" {{ old('minimum_qualification') == 'HND' ? 'selected' : '' }}>HND</option>
                                        <option value="MBA/MSC" {{ old('minimum_qualification') == 'MBA/MSC' ? 'selected' : '' }}>MBA/MSC</option>
                                        <option value="MBBS" {{ old('minimum_qualification') == 'MBBS' ? 'selected' : '' }}>MBBS</option>
                                        <option value="MPhil" {{ old('minimum_qualification') == 'MPhil' ? 'selected' : '' }}>MPhil</option>
                                        <option value="Vocational" {{ old('minimum_qualification') == 'Vocational' ? 'selected' : '' }}>Vocational</option>
                                        <option value="Others" {{ old('minimum_qualification') == 'Others' ? 'selected' : '' }}>Others</option> 
                                    </select>
                                    ...
                                    <div class="flex flex-col md:flex-row md:space-x-2">
                                        ...
                                        <div class="w-full md:w-1/2">
                                            <select name="end_year" id="end_year" class="h-10 py-1 px-2 rounded-md border border-gray-300 hover:border-gray-400 outline-none text-gray-500 placeholder-gray-400 w-full block mt-1">
                                                ...
                                                @endphp
                                                @for ($x = 0; $x <=50; $x++)
                                                    <option value="{{$year - $x}}" {{old('end_year') == $year - $x ? 'selected' : ''}}>{{$year - $x}}</option>
                                                @endfor  
                                            </select>
                                            @error('end_year')
                                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            ...
                            <div class="w-full flex flex-col space-y-3 md:space-y-0 md:flex-row md:space-x-6 mb-3">
                                <div class="w-full">
                                    ...
                                    @error('description')
                                        <p class="text-red-500 text-xs mt-1">{{$message}}</p> 
                                    @enderror
                                    <textarea name="description" id="description" cols="60" rows="12" placeholder="Max. 1000 characters" class="border border-gray-200 w-full outline-none focus:border-gray-400 text-gray-500">{{old('description')}}</textarea>
