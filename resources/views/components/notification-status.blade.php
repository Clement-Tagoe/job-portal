...
<div 
    ...
    x-init="
        ...
        @else
            Livewire.on('companyDetailsUpdated', message => {
            isError = false
            showNotification(message)
            })

            Livewire.on('personalDetailsUpdated', message => {
            isError = false
            showNotification(message)
            })  
            Livewire.on('emailUpdated', message => {
            isError = false
            showNotification(message)
            })
            Livewire.on('passwordChanged', message => {
            isError = false
            showNotification(message)
            })
            Livewire.on('passwordWrong', message => {
            isError = true
            showNotification(message)
            })
            Livewire.on('setCompanyLogo', message => {
            isError = false
            showNotification(message)
            })
            Livewire.on('accountCreated', message => {
            isError = false
            showNotification(message)
            })
        @endif
