    ...
    <div class="w-full mt-16 flex flex-col">
        ...
        <div class="w-full mx-auto">
            <div class="flex flex-col md:flex-row items-center space-x-4 w-11/12 mx-auto mb-8">
                <div class="h-36 w-36 border border-gray-300 rounded-full bg-gray-200 flex items-center justify-center">
                    ...
                    @elseif (!is_null($employer->company_logo))
                        <img class="w-36 h-36" src="{{ asset('storage/' . $employer->company_logo)}}" alt="">  
                    @endif                     
                </div>
                <div>
                    ...
                    <div class="mt-3 text-center md:text-left">
                        <a href="{{route('employers.edit', $employer)}}" class="px-4 py-2 bg-white text-orange-600 font-semibold rounded-md border border-orange-600">Update Profie</a>
                    </div>
                </div>
            </div>
            ...
            <div class="w-11/12 mx-auto">
                <h2 class="text-center text-xl text-gray-700 mt-5">Manage Job Postings</h2>
                                ...
                                <tbody class="bg-white divide-y">
                                   @forelse ($employer->jobs as $job)
                                        <tr class="text-gray-700">
                                            <td class="px-4 py-3 text-sm"><a href="#" class="hover:text-orange-600">{{$job->title}}</a></td>
                                            ...
                                            <td class=" flex px-4 py-3 space-x-2 items-center justify-center">
                                                <a href="{{route('jobs.edit', $job)}}" class="inline-block bg-purple-600 rounded-lg border-2 border-transparent  px-4 py-1 text-center text-sm font-medium leading-5 text-white transition-colors duration-150 hover:bg-purple-700 focus:outline-none focus:ring active:bg-purple-600">
                                                    {{ __('Edit') }}
                                                </a>
